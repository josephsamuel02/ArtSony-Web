...
const StageFive = ({ uploadImage }: cmpData) => {
  const dispatch = useDispatch<AppDispatch>();
  const navigate = useNavigate();

  const userId = useSelector((state: any) => state.Auth.user.user.userId);
  const stateArtData = useSelector((state: any) => state.PostArtwork.post_artwork_draft);
  ...
  const uploadProductionFiles = async (): Promise<void> => {
    ...
    await dispatch(PostArtworkDraft({ production_files: productionFilesUrls }));
  };
  ...
  useEffect(() => {
    dispatch(PostArtworkDraft({ production_files: productionFilesUrls }));
